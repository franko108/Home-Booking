...
<?php     
 ...
 <input   id="day_in_month" type="text" name="day_in_month"  size="9" value="" onkeypress="return numbersonly(this, event)">
...
<tr> 
    ...
    <th><?php echo lang('number_days_short'); ?></th>
...
<tbody> 
	...
	foreach($auto_insert_query->result() as $res){
		...
		if($res->incomeOutcome == 0){
			...
			echo "<td>$res->description</td><td>$res->bookingAmount</td><td>$res->dateMonthPayment</td><td>$res->numberDays</td>
			...
			echo anchor('autoinsert/delete/'.$res->id, 'Delete' , array('class'=>'delete','onclick'=>"return confirm('Delete $res->description ? ')"));
			echo "</td></tr>";
